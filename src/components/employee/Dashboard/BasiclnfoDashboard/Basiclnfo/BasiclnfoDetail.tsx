...
const BasicInfoDetail = ({ staffId }: Props) => {
  ...
  return (
    <Box sx={{ maxWidth: 820, mx: "auto", px: 2, py: 2 }}>
      <Paper sx={{ p: 3, 
        borderRadius: 3, border: "1px solid #dbe5f5" }}>
        <Stack direction="row" 
        ...
        alignItems="center" sx={{ mb: 2 }}>
          ...
          <Stack direction="row" spacing={1}>
            ...
            onClick={() => router.push("/staff/employee/Basiclnfo/list")}>목록</Button>
        ...
        {!loading && !error && !StaffDetail && (
          <Typography>직원 정보를 찾을 수 없습니다.</Typography>
        )}

        {StaffDetail && (
          <Stack spacing={1.25}>
            <Typography><strong>직원번호:</strong> 
            {StaffDetail.staffId}
            </Typography>

            <Typography>
              <strong>부서 ID:</strong> {StaffDetail.deptId}
              </Typography>
