...
//수정
export const updateNursedApi = async (staffId: number | string, nurseReq: NurseUpdateRequest): Promise<ApiResponse<NurseResponse>> => {
  const response = await apiNurse.put<ApiResponse<NurseResponse>>(`/api/nurse/edit/${staffId}`, nurseReq);
  return response.data;
};

//영구 삭제
export const deleteNurseApi = async (staffId: number | string): Promise<ApiResponse<void>> => {
  const response = await apiNurse.delete<ApiResponse<void>>(`/api/nurse/delete/${staffId}`);
  return response.data;
};

//업로드
export async function uploadFileApi(staffId: number | string, file: File): Promise<ApiResponse<FileUploadResDTO>> {
