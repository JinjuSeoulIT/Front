...
export default function PatientList() {
  ...
  const fetchWithFilter = React.useCallback(
    (filter: "ALL" | "ACTIVE" | "INACTIVE") => {
      dispatch(
        patientActions.fetchPatientsRequest(
          filter === "ALL" ? undefined : { statusCode: filter }
        )
      );
    },
    [dispatch]
  ...
  React.useEffect(() => {
    fetchWithFilter(statusFilter);
  }, [fetchWithFilter, statusFilter]);

  const displayListForSelect =
    statusFilter === "ALL"
      ? list
      : statusFilter === "INACTIVE"
        ? list.filter((p) => p.statusCode === "INACTIVE")
        : list.filter((p) => p.statusCode !== "INACTIVE" || !p.statusCode);
  ...
  React.useEffect(() => {
    ...
    if (selected && filtered.some((p) => p.patientId === selected.patientId)) return;
    dispatch(patientActions.fetchPatientSuccess(filtered[0]));
  }, [list, statusFilter, selected, dispatch]);
  ...
  const displayList = displayListForSelect;
  const primary = selected ?? displayList[0] ?? null;

  const totalCount = displayList.length;
  const vipCount = displayList.filter((p) => p.isVip).length;
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <Tabs
        value={statusFilter}
        onChange={(_, v: "ALL" | "ACTIVE" | "INACTIVE") => {
          setStatusFilter(v);
          fetchWithFilter(v);
        }}
        sx={{ borderBottom: 1, borderColor: "divider", minHeight: 40 }}
      >
        <Tab label="활성 환자" value="ACTIVE" />
        <Tab label="비활성 환자" value="INACTIVE" />
        <Tab label="전체" value="ALL" />
      </Tabs>
      {statusFilter === "INACTIVE" && (
        <Typography variant="body2" color="text.secondary" sx={{ mt: -1 }}>
          비활성 환자만 표시됩니다. 환자번호·이름·연락처·생년월일로 검색할 수 있습니다.
        </Typography>
      )}

      <Stack direction="row" spacing={1} sx={{ flexWrap: "wrap" }}>
        <Chip label={`목록 ${totalCount}명`} color="primary" />
